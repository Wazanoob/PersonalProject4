...
public class HummingBridAgent : Agent
{
    ...
    public override void OnActionReceived(ActionBuffers vectorAction)
    {
        ...
        float pitch = rotationVector.x + m_smoothPitchChange * Time.fixedDeltaTime * pitchSpeed;
        if (pitch > 180f) pitch -= 360f;
        pitch = Mathf.Clamp(pitch, -MAX_PITCH_ANGLE, MAX_PITCH_ANGLE);

        float yaw = rotationVector.y + m_smoothYawChange * Time.fixedDeltaTime * yawSpeed;
    ...
    {
        if (trainingMode && (collision.collider.CompareTag("Boundary") || collision.collider.CompareTag("Untagged")))
        {
